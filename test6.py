...
class Camera:
    ...
    def update(self):
        ...
        self.right = norm(np.cross([0, 0, 1], self.forward))  # Глобальный "вверх" — ось Z
...
def rt_render(cam, scene, res):
    ...
    for i in range(ch):
        for j in range(cw):
            u = (j + 0.5) / cw
            v = (i + 0.5) / ch
            rd = cam.forward + (u - 0.5) * cam.width * cam.right + (v - 0.5) * cam.height * -cam.up
            rd = norm(rd)
            color = trace_ray(scene, cam.pos, rd)
            canvas[j, i] = np.clip(color, 0, 255)
    return canvas

class Application:
    def __init__(self):
        pg.init()
        self.RES = self.WIDTH, self.HEIGHT = 100, 100  # Увеличенное разрешение
        self.FPS = 60
        ...
        self.scene1 = Scene()
        self.scene1.add_obj(Sphere([3, 0, -1], 1, [255, 0, 0]))  # Красная сфера
        self.scene1.add_obj(Sphere([4, 2, 0], 1, [0, 0, 255]))   # Синяя сфера
